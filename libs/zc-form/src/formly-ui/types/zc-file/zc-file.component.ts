...
@Component({
  selector: 'zc-file',
  templateUrl: './zc-file.component.html',
  styleUrls: ['./zc-file.component.scss']
})
export class ZcFileComponent extends FieldType implements OnInit {
  files: FileList;
  uploadedFiles: any[];
  uploadFiles = [];
  to: any;
    ...
    this.postFile(this.baseUrl + 'upload/').subscribe(
      res => {
        ...
        if (this.formControl.value && this.to.multiple) {
          ...
          if (this.formControl.value && this.formControl.value.length > 0 && this.formControl.value[0].saved) {
