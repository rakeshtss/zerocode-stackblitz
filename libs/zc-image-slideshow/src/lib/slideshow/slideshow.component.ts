...
export class SlideshowComponent implements OnInit, DoCheck {
  ...
  @Input() jsonParse = false;

  @Output() zcSlideLeft = new EventEmitter<number>();
  @Output() zcSlideRight = new EventEmitter<number>();
  @Output() zcSwipeLeft = new EventEmitter<number>();
  @Output() zcSwipeRight = new EventEmitter<number>();
  ...
  ngOnInit() {
    ...
    if (this.imageUrls && this.imageUrls.length) {
      this.imageUrls.map(img => {
        if (allowTypes.includes(img['contentType'])) {
          img.url = this.appConfigService.settings.fileServer + 'get/' + img.path;
          img.href = this.appConfigService.settings.fileServer + 'get/' + img.path;

          this.primeNgImages.push({ source: img.url, alt: ' ', title: ' ' });
          return img;
        }
      });
  ...
  private slideLeft(oldIndex: number, isSwipe?: boolean): void {
    if (isSwipe === true) {
      this.zcSwipeLeft.emit(this.slideIndex);
    } else {
      this.zcSlideLeft.emit(this.slideIndex);
    }
  ...
  private slideRight(oldIndex: number, isSwipe?: boolean): void {
    if (isSwipe === true) {
      this.zcSwipeRight.emit(this.slideIndex);
    } else {
      this.zcSlideRight.emit(this.slideIndex);
    }
